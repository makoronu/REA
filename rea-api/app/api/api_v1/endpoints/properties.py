...
from app.schemas.property import Property, PropertyCreate, PropertyUpdate, PropertySearchParams
...
def read_properties(
    ...
    properties = property_crud.get_properties(db, skip=skip, limit=limit, search_params=search_params)
    return properties
...
def create_property(property: PropertyCreate, db: Session = Depends(get_db)):
    ...
    if property.homes_id:
        existing_property = property_crud.get_property_by_homes_id(db, homes_id=property.homes_id)
        if existing_property:
            raise HTTPException(status_code=400, detail="Property with this homes_id already exists")
...
def update_property(
    ...
    """物件更新"""
    db_property = property_crud.update_property(db, property_id=property_id, property_update=property)
    if db_property is None:
        raise HTTPException(status_code=404, detail="Property not found")
    return db_property
