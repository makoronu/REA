...
class URLQueue:
    ...
    def _load_state(self):
        ...
        if self.queue_file.exists():
            try:
                ...
                logger.info(f"[{self.site_name}] Loaded {len(self.queue)} URLs from queue")
        ...
        if self.completed_file.exists():
            try:
                ...
                logger.info(f"[{self.site_name}] Loaded {len(self.completed)} completed URLs")
            except Exception as e:
    ...
    def save_state(self):
        ...
        try:
            ...
            logger.debug(f"[{self.site_name}] Saved state: {len(self.queue)} pending, {len(self.completed)} completed")
        except Exception as e:
...
def extract_contractor_info(soup: BeautifulSoup, selectors: Dict[str, str]) -> Dict[str, Any]:
    """
    元請会社情報を汎用的に抽出
    ...
    if company_section:
        ...
        for selector in selectors.get('company_name', '').split(', '):
            if selector:
                ...
                if name_elem:
                    ...
                    contractor_info['contractor_company_name'] = normalize_company_name(raw_name)
                    break
        ...
        if license_selector:
            ...
            if license_elem:
                contractor_info['contractor_license_number'] = license_elem.get_text(strip=True)
        ...
        if contact_selector:
            ...
            if contact_elem:
                contractor_info['contractor_contact_person'] = contact_elem.get_text(strip=True)
    ...
    if not contractor_info.get('contractor_company_name'):
        ...
        if matches:
            # 最初に見つかった会社名を使用
            contractor_info['contractor_company_name'] = normalize_company_name(matches[0])
    
    return contractor_info


def find_next_page_url(soup: BeautifulSoup, current_url: str, selectors: List[str]) -> Optional[str]:
    """
    次ページのURLを汎用的に探す
...
def extract_table_data(soup: BeautifulSoup, target_headers: Dict[str, str]) -> Dict[str, Any]:
    """
    テーブルから指定項目のデータを抽出
...
def batch_process_urls(
    ...
    logger.info(f"Starting batch process: {batch_size} URLs (Pending: {stats['pending']})")
    ...
    for i, url in enumerate(urls, 1):
        ...
        try:
            ...
            if result:
                ...
                logger.success(f"✓ Successfully processed: {result.get('title', 'Unknown')}")
...
def extract_table_data_with_mapping(soup: BeautifulSoup, field_mapping: Dict[str, Any]) -> Dict[str, Any]:
    """
    テーブルデータを汎用的に抽出
...
def extract_price_japanese(soup: BeautifulSoup, selectors: List[str] = None) -> Dict[str, Any]:
    """
    日本語の価格表記を汎用的に抽出
