...
class REAMasterGenerator:
    ...
    def generate_all(self, force_update: bool = False):
        ...
        try:
            ...
            claude_success = self._safe_claude_optimization(
                detail_success, force_update
            )
        ...
        try:
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    str(
                        self.base_path / "scripts/auto_spec_generator/main_generator.py"
                    ),
                ],
                capture_output=True,
                text=True,
    ...
    def _safe_claude_optimization(
        self, detail_success: bool, force_update: bool
    ) -> bool:
        """Claude最適化 - エラー耐性版"""
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    str(
                        self.base_path
                        / "scripts/auto_spec_generator/claude_chunk_generator.py"
                    ),
                ],
                capture_output=True,
                text=True,
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    str(
                        self.base_path
                        / "scripts/auto_spec_generator/claude_integration_generator.py"
                    ),
                ],
                capture_output=True,
                text=True,
...
'''
            ...
            with open(
                shortcode_dir / "expand_shortcode.py", "w", encoding="utf-8"
            ) as f:
    ...
    def _is_cache_valid(
        self, cache_type: str, last_config: dict, valid_hours: int = 24
    ) -> bool:
        """キャッシュ有効性チェック"""
        if not last_config:
            return False
    ...
    def _cache_success(self, operation: str):
        ...
        cache_data = {"timestamp": datetime.now().isoformat(), "success": True}
...
"""
    ...
    def _print_summary(
        self, basic: bool, detail: bool, claude: bool, guide: bool, shortcode: bool
    ):
        """実行結果サマリー"""
        print(f"\n📋 REA統合仕様書生成結果:")
        print("─" * 50)
...
def main():
    ...
    parser.add_argument("--quick", action="store_true", help="クイック更新（差分のみ）")
