    ...
    def _generate_group_specs(self, group_name: str, group_info: dict, inspector):
        ...
        for table_name in group_info["tables"]:
            if table_name in inspector.get_table_names():
                self._generate_single_table_detail(
                    table_name, group_name, group_dir, inspector
                )

    def _generate_group_overview(
        self, group_name: str, group_info: dict, group_dir: Path
    ):
        """機能グループ概要生成"""
...
"""
    ...
    def _generate_single_table_detail(
        self, table_name: str, group_name: str, group_dir: Path, inspector
    ):
        """単一テーブルの詳細仕様生成"""
        columns = inspector.get_columns(table_name)
...
"""
        ...
        for i, column in enumerate(columns, 1):
            ...
            description = self._get_column_description_detailed(
    ...
    def _generate_constraints_section(
        self, table_name: str, foreign_keys: list, indexes: list
    ) -> str:
        """制約・インデックス情報セクション生成"""
        content = "\n## 🔗 制約・インデックス情報\n"
    ...
    def _get_column_description_detailed(
    ...
    def _get_related_api_endpoints(self, table_name: str, group_name: str) -> list:
        ...
        endpoints = {
            ...
            "properties_pricing": [
                {
                    "method": "GET",
                    "path": "/api/v1/properties/{id}/pricing",
                    "description": "価格情報取得",
                },
                {
                    "method": "PUT",
    ...
    def _print_summary(self):
        ...
        for group_name, group_info in self.table_groups.items():
            ...
            if group_dir.exists():
                ...
                print(
                    f"{group_info['icon']} {group_info['title']}: {len(md_files)}ファイル"
                )
