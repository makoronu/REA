...
class DBConnectionUnifier:
    def __init__(self):
        ...
        self.backup_dir = self.project_root / f"backup_db_unify_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        self.files_to_fix = []
...
"""
    ...
    def fix_rea_scraper_settings(self, filepath):
        ...
        if '# DB接続はshared/database.pyを推奨' not in content:
            content = '''# 注意: 可能な限りDB接続はshared/database.pyを使用してください
# from shared.database import READatabase
# conn = READatabase.get_connection()
...
"""
    ...
    def fix_python_files(self):
        ...
        for filepath in self.files_to_fix:
            ...
            elif 'scripts/auto_spec_generator' in str(rel_path) and 'database_generator' not in str(rel_path):
