...
def create_new_tables(cur):
    ...
    cur.execute("CREATE INDEX idx_property_images_property ON property_images(property_id)")
    cur.execute("CREATE INDEX idx_property_images_order ON property_images(property_id, display_order)")
    print("  ✅ インデックス作成完了")

def show_new_structure(db):
    """新しいテーブル構造を表示"""
    tables = ['properties', 'land_info', 'building_info', 'amenities', 'property_images']
