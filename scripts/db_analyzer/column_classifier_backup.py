        ...
        self.classification_rules = {
            'core': {
                ...
                'keywords': [
                    ...
                    'building_name_kana', 'property_name_public', 'total_units',
                    'vacant_units', 'created_at', 'updated_at', 'source_site',
                    'extraction_confidence', 'data_quality_score', 'original_data'
                ],
            ...
            'pricing': {
                'priority': 3,
                'keywords': [
                    'rent_price', 'price_status', 'tax', 'tax_amount', 'price_per_tsubo',
                    'common_management_fee', 'full_occupancy_yield', 'current_yield',
                    'housing_insurance', 'land_rent', 'repair_reserve_fund',
                    'parking_fee', 'contract_period'
                ],
                'patterns': [r'.*price.*', r'.*fee.*', r'.*yield.*', r'.*tax.*', r'.*rent.*'],
                'icon': '💰',
                'title': '価格・収益テーブル',
            ...
            'location': {
                'priority': 4,
                'keywords': [
                    'postal_code', 'address_code', 'address_name', 'address_detail',
                    'latitude_longitude', 'train_line', 'station', 'bus_stop_name',
                    'bus_time', 'walk_time'
                ],
                'patterns': [r'.*address.*', r'.*train.*', r'.*station.*', r'.*bus.*', r'.*walk.*'],
                'icon': '📍',
                'title': '所在地・交通テーブル',
            ...
            'building': {
                'priority': 5,
                'keywords': [
                    'building_structure', 'building_age', 'construction_year',
                    'construction_month', 'total_floors', 'floor_plan',
                    'exclusive_area', 'balcony_area', 'floor_number'
                ],
                'patterns': [r'.*building.*', r'.*floor.*', r'.*construction.*', r'.*area.*'],
                'icon': '🏗️',
                'title': '建物情報テーブル',
            ...
            'facilities': {
                'priority': 6,
                'keywords': [
                    'shopping_street_distance', 'drugstore_distance', 'park_distance',
                    'bank_distance', 'other_facility_name', 'other_facility_distance'
                ],
            ...
            'contract': {
                'priority': 7,
                'keywords': [
                    'contract_type', 'property_publication_type', 'contractor_company_name',
                    'contractor_contact_person', 'contractor_phone', 'contractor_email',
                    'contractor_address', 'contractor_license_number'
                ],
            ...
            'renovation': {
                'priority': 8,
                'keywords': [
                    'renovation_water', 'renovation_interior', 'renovation_exterior',
                    'renovation_common_area', 'renovation_notes'
                ],
            ...
            'energy': {
                'priority': 9,
                'keywords': [
                    'energy_consumption_min', 'energy_consumption_max',
                    'insulation_performance_min', 'insulation_performance_max',
                    'utility_cost_min', 'utility_cost_max'
                ],
...
"""
        ...
        for category, columns in self.categorized_columns.items():
            ...
            sql += f"INSERT INTO {table_name} (property_id, {', '.join(column_names)})\n"
...
"""
        ...
        with open(self.output_dir / "README_EXECUTION_GUIDE.md", 'w', encoding='utf-8') as f:
            f.write(guide)
