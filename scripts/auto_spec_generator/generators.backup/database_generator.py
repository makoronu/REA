        ...
        try:
            ...
            try:
                ...
                if health['status'] != 'healthy':
                    self.print_status(f"⚠️ DB接続警告: {health.get('error', 'Unknown error')}")
...
"""
    ...
    def _pattern_match_purpose(self, table_name: str, columns: list, record_count: int) -> str:
        """パターンマッチングによる用途推定"""
        ...
        # ログテーブルパターン  
        if 'created_at' in column_names and ('log' in table_name.lower() or record_count > 1000):
            return 'ログテーブル'
            
        # 画像テーブルパターン
        image_keywords = ['image', 'photo', 'picture', 'file_name', 'url']
        if any(word in col_name for col_name in column_names for word in image_keywords):
            return '画像管理テーブル'
        
        # 価格テーブルパターン
        price_keywords = ['price', 'cost', 'fee', 'amount', 'rent']
        if any(word in col_name for col_name in column_names for word in price_keywords):
            return '価格・料金テーブル'
