...
class SharedLibraryAnalyzer:
    ...
    def generate(self) -> Dict[str, Any]:
        ...
        for file_path in python_files:
            ...
            try:
                ...
                total_functions = len(classes.get(list(classes.keys())[0], {}).get("methods", [])) if classes else 0
    ...
    def _extract_classes_detailed(self, content: str) -> Dict[str, Dict]:
        ...
        for match in class_matches:
            ...
            # クラスのdocstring抽出
            docstring = ""
            docstring_match = re.search(r'^\s*"""(.*?)"""', class_body, re.DOTALL | re.MULTILINE)
            if docstring_match:
            ...
            for method_match in method_matches:
                ...
                return_type = f"-> {method_match.group(3).strip()}" if method_match.group(3) else ""
                
                # メソッドのdocstring抽出
                method_docstring = ""
                method_start = method_match.end()
                remaining_body = class_body[method_start:]
                method_doc_match = re.search(r'^\s*"""(.*?)"""', remaining_body, re.DOTALL | re.MULTILINE)
                if method_doc_match:
        ...
        for i, line in enumerate(lines):
            ...
            if re.match(r'\s*def\s+', line):
                ...
                if match:
                    ...
                    return_type = f"-> {match.group(3).strip()}" if match.group(3) else ""
                                    ...
                                    for k in range(j + 1, min(j + 5, len(lines))):
                                        if k < len(lines):
                                            ...
                                            if '"""' in doc_line or "'''" in doc_line:
                                                doc_lines.append(doc_line.split('"""')[0].split("'''")[0])
                                                break
    ...
    def _generate_complete_reference(self, analysis_results: Dict[str, Any]):
        ...
        for i, (filename, analysis) in enumerate(analysis_results.items(), 1):
            if "error" in analysis:
                ...
                content.append(f"| {i} | {filename} | {size_kb}KB | {class_count} | {total_func_count} | {line_count} | {doc_summary} |")
                ...
                for class_name, class_info in classes.items():
                    ...
                    if methods:
                        ...
                        for method in methods:
                            ...
                            if method.get("docstring"):
                                content.append(f"  - 📝 {method['docstring'].split(chr(10))[0]}")
                            content.append("")
        ...
        # サマリー
        total_classes = sum(len(a.get("classes", {})) for a in analysis_results.values() if "error" not in a)
        total_functions = 0
        total_lines = sum(a.get("line_count", 0) for a in analysis_results.values() if "error" not in a)
