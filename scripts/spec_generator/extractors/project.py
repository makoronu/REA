...
class ProjectExtractor:
    ...
    def _get_project_structure(self):
        """プロジェクト構造を取得"""
        modules = ['rea-api', 'rea-scraper', 'rea-admin', 'rea-search', 'rea-publisher', 'rea-wordpress']
        structure = {
            'modules': {},
            'total_files': 0,
            'total_lines': 0
    ...
    def _analyze_module(self, module_path):
        ...
        info['total_py_files'] = len([f for f in py_files if 'venv' not in str(f) and '__pycache__' not in str(f)])
        ...
        # 設定ファイルの確認
        config_files = ['.env', '.env.example', 'requirements.txt', 'package.json', 'docker-compose.yml']
