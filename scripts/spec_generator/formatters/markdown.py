...
class MarkdownFormatter:
    ...
    def format(self, spec_data):
        ...
        self._add_implementation_status(spec_data.get('project', {}).get('implementation_status', {}))
    ...
    def _add_project_structure(self, structure):
        ...
        for module_name, info in structure.get('modules', {}).items():
            ...
            if info.get('exists'):
                ...
                self.lines.append(f"- Directories: {', '.join(info.get('directories', []))}")
    ...
    def _add_properties_columns(self, column_groups):
        ...
        for group_name, columns in column_groups.items():
            ...
            for col in columns[:10]:
                self.lines.append(f"| {col['name']} | {col['type']} | {col['nullable']} | {col['japanese_label']} |")
            
            if len(columns) > 10:
                self.lines.append(f"| ... | ({len(columns) - 10} more columns) | ... | ... |")
    ...
    def _add_api(self, api_data):
        ...
        if endpoints:
            ...
            self.lines.append("|--------|------|---------|")
            for ep in endpoints:
                self.lines.append(f"| {ep['method']} | {ep['path']} | {ep['summary']} |")
    ...
    def _add_implementation_status(self, impl_data):
        ...
        if impl_data.get('in_progress'):
            ...
            self.lines.append(f"\n### 🔄 In Progress")
            self.lines.append(f"**{prog['phase']}: {prog['name']}** ({prog['progress']})")
