...
| Seg | 対象 | 件数 | 目的 | 優先度 | 状態 |
|-----|------|------|------|--------|------|
| **1** | バリデーション | 5 | ユーザー問題解決 | 最高 | 計画中 |
| **2** | ステータス・選択肢・UI色 | 40+ | マスタ化 | 高 | 未着手 |
| **3** | 権限・ストレージ・ページサイズ・検索距離 | 30+ | 一元化 | 中 | 未着手 |
| **4** | テーブル名・APIパス・学校コード | 90+ | 構造統一 | 中 | 未着手 |
| **5** | 日付・placeholder・エラーメッセージ | 85+ | UI/UX統一 | 低 | 未着手 |
...
### Seg3: 権限・ストレージ・ページサイズ・検索距離（30+件）
...
### Seg5: 日付・placeholder・エラーメッセージ（85+件）
