...
**最終更新**: 2026-01-03
**現在フェーズ**: HOMES入稿 / ZOHO連携

### 直近の完了
- 公開時バリデーション「該当なし」対応（本番デプロイ済）✅
- プロトコル違反修正 Step 1-7, Phase 3-5（本番デプロイ済）✅
- ユーザー管理機能（本番デプロイ完了）✅
- 認証フロントエンド（ログイン・パスワードリセット）✅
- チラシ・マイソク基盤（フェーズ4.7）✅
- データ設計正規化（ENUM廃止、マスター統合）✅
...
### Phase 0: 準備（dev-protocol必須） ✅ 完了

| # | 項目 | 状態 |
|---|------|------|
| 0-1 | DBバックアップ取得 | [x] |
| 0-2 | `rea-flyer/README.md` 作成 | [x] |
| 0-3 | 既存共通処理の棚卸し（`shared/constants.py`確認） | [x] |
| 0-4 | 依存ライブラリ調査（svgwrite, cairosvg, Pillow） | [x] |
...
|---|------|------|
| 1-1 | ディレクトリ構造作成 | [x] |
| 1-2 | `config/output_settings.yaml` 作成 | [x] |
| 1-3 | `config/field_mappings.yaml` 作成（column_labels連携） | [x] |
| 1-4 | `shared/real_estate_utils.py` にフォーマット関数追加 | [x] |
| 1-5 | `generators/base.py` 基底クラス作成 | [x] |
| 1-6 | 単体テスト環境構築 | [x] |
| 1-7 | **Selenium動作確認**（APIエンドポイント疎通） | [x] |
...
| # | 項目 | 状態 |
|---|------|------|
| 2-1 | `config/templates.yaml` マイソク定義 | [x] |
| 2-2 | `templates/maisoku/land.svg` 土地用テンプレート | [x] |
| 2-3 | `generators/maisoku.py` 土地用生成ロジック | [x] |
| 2-4 | **実務テスト**: 実物件で土地マイソク生成→印刷屋確認 | [ ] 後日 |
| 2-5 | `templates/maisoku/detached.svg` 戸建用テンプレート | [x] |
| 2-6 | `templates/maisoku/apartment.svg` マンション用テンプレート | [x] |
| 2-7 | `templates/maisoku/investment.svg` 収益物件用テンプレート | [x] |
| 2-8 | **Selenium動作確認**（各テンプレート生成） | [x] |
...
| # | 項目 | 状態 |
|---|------|------|
| 3-1 | `config/templates.yaml` チラシ定義追加 | [x] |
| 3-2 | `templates/chirashi/single.svg` A4全面テンプレート | [x] 動的生成 |
| 3-3 | `generators/chirashi.py` 生成ロジック | [x] |
| 3-4 | 写真配置ロジック（property_imagesから自動選択） | [ ] 次Phase |
| 3-5 | **Selenium動作確認** | [x] |

---

### Phase 4: チラシ生成（複数件） ✅ 完了

| # | 項目 | 状態 |
|---|------|------|
| 4-1 | `templates/chirashi/dual.svg` 2件用テンプレート | [x] 動的生成 |
| 4-2 | `templates/chirashi/grid.svg` 4件グリッド | [x] 動的生成 |
| 4-3 | 物件数→レイアウト自動選択ロジック | [x] |
| 4-4 | **Selenium動作確認** | [x] |
...
| # | 項目 | 状態 |
|---|------|------|
| 5-1 | `rea-api/app/api/v1/endpoints/flyer.py` 作成 | [x] |
| 5-2 | rea-admin 物件詳細画面に「マイソク生成」ボタン追加 | [x] |
| 5-3 | rea-admin 物件一覧画面に「チラシ生成」ボタン追加（複数選択） | [x] |
| 5-4 | プレビューモーダル実装 | [x] |
| 5-5 | テンプレート選択UI | [x] 物件種別から自動選択（メタデータ駆動） |
| 5-6 | **Selenium E2Eテスト** | [x] 基本確認済み |
...
## フェーズ4.8: HOMES入稿 🔥 現在優先

**目的**: REAの物件データをLIFULL HOME'S形式でCSV出力し、ポータルサイトに入稿可能にする
...
- ホームズへの物件登録は「LIFULL HOME'S Manager」でCSVアップロードが一般的
- API連携（HOME'S PRO）はパートナー契約・審査が必要で時間がかかる
- CSVは今すぐ実装可能、かつ実務で最もよく使われている

### 既存資産

- `master_options`に**46カテゴリ**のホームズコード登録済み（source='homes'）
- ホームズCSV仕様書v4.3.2から抽出済み
- `portal_mappings`テーブル設計済み（REAコード↔ポータルコード変換用）
...
| # | 項目 | 状態 |
|---|------|------|
| 3-1 | 物件一覧に「HOMES出力」ボタン追加 | [ ] |
| 3-2 | 出力対象物件の選択UI（チェックボックス） | [ ] |
| 3-3 | 出力設定モーダル（含める項目、画像有無など） | [ ] |
| 3-4 | CSVダウンロード処理 | [ ] |
| 3-5 | **Selenium動作確認** | [ ] |

---

### Phase 4: バリデーション・エラー処理

| # | 項目 | 状態 |
|---|------|------|
| 4-1 | 必須フィールド欠損チェック | [ ] |
| 4-2 | 文字数制限チェック（ホームズ仕様準拠） | [ ] |
| 4-3 | 画像枚数・サイズ制限チェック | [ ] |
| 4-4 | エラーレポート出力（何が足りないか表示） | [ ] |
...
| 優先度 | 機能 | 備考 |
|--------|------|------|
| 高 | SUUMO CSV出力 | 同様の仕組みで対応可能 |
| 高 | athome CSV出力 | 同様の仕組みで対応可能 |
| 中 | レインズ連携 | 専用フォーマット |
| 低 | HOME'S PRO API連携 | パートナー契約後 |

---

## フェーズ4.9: ユーザー管理機能 🔥 現在優先
...
### Phase 1: 基本機能 ✅ 完了（2025-12-17）

| # | 項目 | 状態 |
|---|------|------|
| 1-1 | ユーザー一覧表示（自社ユーザーのみ） | [x] |
| 1-2 | 新規ユーザー作成（名前・メール・権限） | [x] |
| 1-3 | ウェルカムメール送信（パスワード設定リンク） | [x] |
| 1-4 | ユーザー有効化/無効化 | [x] |
| 1-5 | 管理者のみアクセス可能（minLevel=80） | [x] |
| 1-6 | デフォルト権限＝一般ユーザー | [x] |
| 1-7 | 本番デプロイ（deploy-protocol準拠） | [x] |

---

### Phase 2: v1.0 拡張 🔄 次回着手

| # | 項目 | 状態 | 工数 |
|---|------|------|------|
| 2-1 | 自分自身の操作制限（無効化・降格不可） | [ ] | 30分 |
| 2-2 | 削除確認ダイアログ | [ ] | 30分 |
| 2-3 | パスワード未設定表示（「招待中」バッジ） | [ ] | 30分 |
| 2-4 | 招待メール再送信 | [ ] | 1時間 |
| 2-5 | パスワードリセット送信（管理者から） | [ ] | 1時間 |
| 2-6 | ユーザー編集モーダル（名前・権限変更） | [ ] | 1時間 |
...
## フェーズ4.10: 公開時バリデーション 🔥 現在優先

**目的**: 公開ステータスに変更する際、必須項目が未入力なら公開を拒否し、入力を促す
...
- 準備・査定段階では全項目入力不要
- 公開段階ではポータル掲載に必要な情報が必須
- 不完全な物件が公開されるのを防ぎ、データ品質を担保
...
| カテゴリ | フィールド | 条件 |
|----------|-----------|------|
| 基本 | property_name | 空でない |
| 基本 | property_type | 'other'以外 |
| 価格 | sale_price | > 0 |
| 所在地 | prefecture | 空でない |
| 所在地 | city | 空でない |
| 所在地 | address | 空でない |
| アクセス | transportation | 1件以上 |
| 建物 | exclusive_area | > 0（マンションの場合） |
| 土地 | land_area | > 0（土地・戸建の場合） |
...
| # | 項目 | 状態 |
|---|------|------|
| 2-1 | `validators/publication.py` 作成 | [x] |
| 2-2 | ステータス変更APIに組み込み | [x] |
| 2-3 | エラーレスポンス形式統一 | [x] |

**実装イメージ**
```python
def validate_for_publication(property_id: int) -> list[str]:
    """公開可否チェック。エラーリストを返す"""
    errors = []
    if not property.sale_price:
        errors.append("販売価格が未設定です")
    if not property.prefecture:
        errors.append("都道府県が未設定です")
    # ...
    return errors
```
...
### Phase 4: テスト・デプロイ（0.5日）

| # | 項目 | 状態 |
|---|------|------|
| 4-1 | ローカルテスト（正常系・異常系） | [x] |
| 4-2 | Seleniumテスト | [x] |
| 4-3 | 本番デプロイ | [x] ✅ 2026-01-03完了 |
...
### セキュリティ
- [ ] **レート制限（ログインAPI）** ← ブルートフォース対策、slowapi使用
- [ ] JWT有効期限短縮（480分→60分）
- [ ] CORS設定（本番ドメインのみ許可）
- [ ] HTTPS強制
- [ ] 認証・認可の強化
