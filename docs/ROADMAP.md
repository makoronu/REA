...
| # | 項目 | 状態 |
|---|------|------|
| 1 | ダブり解消：最寄駅・バス・周辺施設を所在地タブに統一、設備タブから削除 | [ ] |
| 2 | ダブり解消：物件種別を基本タブのみに統一 | [ ] |
| 3 | 不要フィールド削除：物件名カナ | [ ] |
| 4 | 不要フィールド削除：画像タブのシステム管理セクション、ファイルパス、URL系 | [ ] |
| 5 | ボタン配置：公開を左、登録を右に | [ ] |
| 6 | 保存ボタンを上部にも追加 | [ ] |
| 7 | タブ順序変更：所在地を最初に | [ ] |
| 8 | 最終更新日時を一番上に小さく表示 | [ ] |
...
| # | 項目 | 状態 |
|---|------|------|
| 9 | 取引態様をラジオ化、「仲介」選択時のみ元請会社入力欄表示 | [ ] |
| 10 | ステータス名変更：「案件ステータス」に統一 | [ ] |
| 11 | ステータス選択肢：査定中/保留/販売準備/販売中/商談中/成約済み/販売終了/取り下げ/他決 | [ ] |
| 12 | 建ぺい率・容積率・用途地域：自動取得値→手入力欄に反映（編集可能） | [ ] |
| 13 | 物件種別による設備表示制御（土地・駐車場は建物設備非表示） | [ ] |
