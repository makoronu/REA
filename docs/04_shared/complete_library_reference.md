...
|----|------------|--------|----------|----------|------|------|
| 1 | config.py | 0KB | 0 | 0 | 1 | - |
| 2 | scrapers_common.py | 15KB | 1 | 25 | 477 | REAスクレイピング共通処理 |
| 3 | formatters.py | 8KB | 0 | 9 | 315 | REAフォーマット処理統一システム |
| 4 | real_estate_utils.py | 6KB | 0 | 8 | 251 | REA不動産業務専用ユーティリティ |
| 5 | validators.py | 0KB | 0 | 0 | 1 | - |
| 6 | database.py | 12KB | 0 | 10 | 371 | REA シンプルデータベース接続 |
| 7 | constants.py | 0KB | 0 | 0 | 1 | - |
| 8 | logger.py | 0KB | 0 | 0 | 1 | - |
| 9 | system_utils.py | 0KB | 0 | 0 | 1 | - |
| 10 | exceptions.py | 0KB | 0 | 0 | 1 | - |
| 11 | path_utils.py | 1KB | 0 | 2 | 44 | REAプロジェクトパス自動設定ユーティリティ |
...
- `from shared.formatters import normalize_company_name, clean_phone_number, normalize_address`
...
#### `extract_contractor_info(soup: BeautifulSoup, selectors: Dict[str, str]) -> Dict[str, Any]`

**説明**:
```
元請会社情報を汎用的に抽出  Args: soup: BeautifulSoupオブジェクト
```

#### `find_next_page_url(soup: BeautifulSoup, current_url: str, selectors: List[str]) -> Optional[str]`

**説明**:
```
次ページのURLを汎用的に探す  Args: soup: BeautifulSoupオブジェクト
```

#### `extract_table_data(soup: BeautifulSoup, target_headers: Dict[str, str]) -> Dict[str, Any]`

**説明**:
```
テーブルから指定項目のデータを抽出  Args: soup: BeautifulSoupオブジェクト
```
...
#### `extract_table_data_with_mapping(soup: BeautifulSoup, field_mapping: Dict[str, Any]) -> Dict[str, Any]`

**説明**:
```
テーブルデータを汎用的に抽出  Args: soup: BeautifulSoupオブジェクト
```

#### `extract_price_japanese(soup: BeautifulSoup, selectors: List[str] = None) -> Dict[str, Any]`

**説明**:
```
日本語の価格表記を汎用的に抽出  Returns: dict: {'price': 価格(数値), 'price_raw': 元の文字列}
```
...
#### `calculate_yield(price: Optional[float], monthly_rent: Optional[float]) -> Optional[float]`

**説明**:
```
表面利回りを計算  Args: price: 物件価格（円）
```
...
#### `execute_query_dict(cls, query: str, params: Optional[tuple] = None) -> List[Dict[str, Any]]`

**説明**:
```
クエリ実行（辞書形式）  結果を辞書形式で返すため、カラム名でアクセス可能。 実用的なデータ処理に最適。
```
