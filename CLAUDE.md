...
| 項目 | 内容 |
|------|------|
| 作業中 | 不動産情報ライブラリAPI統合（APIキー待ち） |
| 完了 | 法規制・災害リスクデータ調査、ROADMAP更新 |
| 残り | API申請→キー取得→防火地域MAP重ね→災害ハザード実装 |
| 最終更新 | 2025-12-12 |
| 備考 | **次回最優先**: https://www.reinfolib.mlit.go.jp でAPI申請。取得後`.env`に`REINFOLIB_API_KEY=xxx`設定 |
