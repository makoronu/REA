...
|------|------|
| 作業中 | なし |
| 完了 | 登記取込機能、ZOHO再インポート2370件、郵便番号自動補完、編集画面データ表示修正（/properties/{id}/full追加、RoadInfoEditor修正） |
| 残り | データ表示修正タスク35-40（住所上書き問題）→ ZOHO画像同期、定期同期 |
| 最終更新 | 2025-12-14 23:30 |
| 備考 | **住所データ上書き問題（明日対応）**: land_infoの同名カラム（postal_code, prefecture, city, address）がNULLでpropertiesの値を上書き。詳細は`docs/logs/2025-12-14-edit-form-fix.md` |
