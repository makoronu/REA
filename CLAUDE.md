...
|------|------|
| 作業中 | なし |
| 完了 | **ユーザー管理機能（本番デプロイ済み）**、パスワードリセット機能、認証フロントエンド |
| 残り | **ユーザー管理v1.0拡張（下記6機能）**、HOMES入稿、ZOHO画像同期 |
| 最終更新 | 2025-12-17 |
| 備考 | **ユーザー管理**: /settings/users（管理者のみ）。デフォルト権限=一般ユーザー。**本番URL**: https://realestateautomation.net/ |

### ユーザー管理 v1.0 実装計画（次回セッション用）

| 優先度 | 機能 | 理由 | 工数 |
|--------|------|------|------|
| 1 | 自分自身の操作制限 | 自分を無効化・降格すると管理者不在 | 30分 |
| 2 | 削除確認ダイアログ | 誤操作防止 | 30分 |
| 3 | パスワード未設定表示 | 「招待中」バッジ | 30分 |
| 4 | 招待メール再送信 | メール届かない時用 | 1時間 |
| 5 | パスワードリセット送信 | PW忘れ対応 | 1時間 |
| 6 | ユーザー編集モーダル | 名前・権限変更（優先度低） | 1時間 |

**v1.1以降**: 検索・フィルタ、操作履歴、一括操作
