...
|------|------|
| 作業中 | なし |
| 完了 | **ヘッダーテナント名表示+開発者ログイン追加**、パスワードリセット機能、本番デプロイ、認証フロントエンド |
| 残り | **本番デプロイ（SSH接続待ち）**、HOMES入稿、ZOHO画像同期 |
| 最終更新 | 2025-12-17 |
| 備考 | **本番URL: https://realestateautomation.net/** ヘッダーにテナント名+ログアウト追加済み。開発者ログイン: `/dev-login`（ダークテーマ）。GitHubにはプッシュ済み、サーバーデプロイはSSH接続後に実行。 |
