...
**Claudeへ: 仕様が不明確な場合、方針に迷う場合は、ユーザーに確認しろ。**

- ツール実行の承認を毎回求めるのは不要（時間の無駄）
- でも仕様確認、方針確認など**必要な質問はすべき**
- 曖昧なまま進めて間違えるより、確認して正しく進める方がいい

**「質問するな」とは言っていない。必要な確認は当然しろ。**
...
|------|------|
| 作業中 | なし |
| 完了 | ユーザー管理機能、パスワードリセット、column_labelsグループ再編成、APIキャッシュ制御 |
...
**Claudeへ: 本番環境への変更は必ずこのスクリプトを使え。例外なし。**
...
以下のコマンドを直接実行するな：
- ❌ `ssh rea-conoha "git pull"`
- ❌ `ssh rea-conoha "npm run build"`
- ❌ `ssh rea-conoha "systemctl restart"`

### deploy.shがやること

1. TypeScriptエラーチェック（失敗したら中止）
2. git push
3. 本番DBバックアップ
4. 設定ファイル保護（.env等をバックアップ）
5. git pull
6. 設定ファイル復元
7. npm install && build
8. API再起動
...
- 設定ファイルを上書きしてメールが使えなくなる
- バックアップを忘れてデータが消える
- ヘルスチェックを忘れて障害に気づかない
- 毎回同じミスを繰り返す

**スクリプトを使えば全部自動でやる。手動でやるな。**
