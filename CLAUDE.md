...
- **MI Seg N: 不動産ジャパン パイプライン統合**（コミット: d485bf9、MIリポジトリ）
  - main.py: SITE_MODULESにfudousan_japan追加（1行）
  - daily_scrape.sh: 複数サイト対応（SITES配列ループ、set -euo→set -uo、サイト別ログ）
  - systemd: TimeoutStartSec 3600→7200、Description更新
  - scrape_sources DB: 札幌全10区×3種別登録（id=10更新tochi, id=11新規kodate, id=12新規mansion）
  - 物件数概算: tochi=1,514 / kodate=1,145 / mansion=1,452 = 合計4,111件
  - 未デプロイ（Contabo直接配置のみ、systemd変更済み）

### 今日完了した作業（2026-02-27 Seg H5: bulk-upsert APIデプロイ）

- **Seg H5: Contabo→ConoHa bulk-upsert API同期デプロイ**（コミット: 154c109, a165264, c6fb091, 0abf609）
  - scraper_sync.py新規（165行）: POST /properties/bulk-upsert エンドポイント
  - generic.py: GenericCRUD.create()にcommitパラメータ追加（複数テーブル原子性確保）
  - shared/database.py: READatabase._load_env() os.environ.setdefault化（systemd環境変数上書き防止）
  - server/rea-scr.service: EnvironmentFile=/opt/REA/.env.scr追加（DB_NAME=rea_scraper確実上書き）
  - ConoHa rea_scraper DB: organizations/m_roles/usersにサービスアカウント作成
  - ConoHa rea_scraper DB: property_urlカラム+UNIQUEインデックス追加
  - Contabo: MI .env更新（REA_SCR_API_URL/USER/PASSWORD）、コードrsync、httpxインストール
  - E2Eテスト: CREATE→UPDATE→Contabo外部経由 全パス
  - 既知制限: building_structure/room_typeがINTEGER型のためスクレイパーの文字列は要コード変換（MI側対応予定）

### 今日完了した作業（2026-02-25 Seg H3 + MI Seg H/H2/H4）

- **Seg H3: scr.realestateautomation.net サブドメイン構築**（コミット: 343ae41、REAリポジトリ）
  - server/rea-scr.service: systemdサービス定義（port 8008, DB_NAME=rea_scraper上書き）
  - server/nginx-scr.conf: nginxサーバーブロック（セキュリティヘッダー付き）
  - scripts/migrations/create_rea_scraper_db.sql: DB作成+price_history/listing_status_history
  - deploy.yml: rea-scr再起動+ヘルスチェック追加（ステップ番号9/9統一）
  - 手動セットアップ待ち: DNS/DB作成/スキーマコピー/systemd/nginx/SSL/CORS

- **MI Seg H: プロキシローテーション**（コミット: 082d6bf、MIリポジトリ）
- **MI Seg H2: マルチワーカー並列処理**（コミット: 6901a57、MIリポジトリ）
- **MI Seg H4: MI→rea_scraper同期サービス**（コミット: 118643f、MIリポジトリ）
  - sync/sync_to_rea.py: scraped_properties→properties/land_info/building_info UPSERT
  - sync/field_mapping.py: 3テーブル振り分け定数
  - main.py: sync-to-rea CLIコマンド追加
  - デプロイ・インフラ情報: market-intelligence/docs/deploy_infrastructure.md

### 今日完了した作業（2026-02-25 MI Seg G: 全量検査12件修正）

- **MI Seg G: 全量検査12件修正**（コミット: 35291c7、market-intelligenceリポジトリ）
  - C1: 会社名抽出追加（問合せ先テキストから、th/td外のp要素検索）
  - C2: 住所パース四日市市等の貪欲マッチ修正（非貪欲→市は貪欲/区町村は非貪欲）
  - C3: scheduler.py fail_session JSONB型修正（Json()ラッパー追加）
  - C4: scraper.pyデッドコード削除（detect_disappeared import, seen_ids変数）
  - H5: WAFチャレンジ失敗時ドライバー解放追加（リソースリーク防止）
  - H6: dedup_keyからprice除外（価格変更でキーが変わる問題解消）
  - M2: HEADLESSデフォルトfalse→true（本番でGUI起動防止）
  - M3: User-Agent Chrome 121→131更新（ブロック誘発防止）
  - M4: db.py utcnow()デッドコード削除
  - M6: HTMLファイル名listing_id抽出改善（URL末尾→/b-(\d+)/パターン+フォールバック）
  - M7: RAW_HTML_DIR相対パス→絶対パス化（起動ディレクトリ依存解消）
  - M9: DB_PASSWORD未設定時警告追加
  - バッチテスト: 新規3件INSERT→会社名全件抽出成功
  - 残: Seg H（セッション統合・キュー改善）、Seg I（current_statusマッピング）

### 今日完了した作業（2026-02-25 MI Seg F: E2Eバッチテスト）

- **MI Seg F: E2Eバッチテスト完了**（コミット: 5db8c3c、market-intelligenceリポジトリ）
  - URL収集: HOMES戸建3ページ巡回→90件キュー登録
  - バッチ処理: 10件→全件INSERT成功（sale_price/address NULL率0%）
  - 差分更新: 同一3件再取得→updated（INSERT重複なし、last_seen_at更新確認）
  - 価格変更: 手動DB変更→再取得→price_historyに+100万円記録
  - 消失検知: seen 3件/active 13件→10件正常消失判定
  - 追加3件処理（合計13件DB保存）
  - 修正4件:
    - block_detector.py: CAPTCHA誤検知（"robot"→"i'm not a robot"、metaタグマッチ防止）
    - registrar.py: JSONB型city_planningにJson()ラップ
    - registrar.py: property_typeをUPDATE_COLUMNSに追加
    - scraper.py: process_batchから消失検知を除去（部分バッチ誤判定防止）
  - 未実装: scrape_sessions記録（セッション統計）

### 今日完了した作業（2026-02-24 Seg 27）

- **Seg 27: 現況（current_status）土地用オプション追加**（コミット: 0f043bb、DB変更のみ・未適用）
  - master_options INSERT 4件: rea_4=未完成, rea_5=更地, rea_6=古屋あり, rea_7=古屋あり更地引渡可
  - master_options DELETE 2件: source='REA'孤立エントリ（0=未設定, 9=その他重複）
  - properties UPDATE: current_status=0（未設定）146件→NULL化
  - column_labels UPDATE: input_type multi_select→radio, min_selections→NULL
  - HOMES仕様書CSV No.184準拠（土地: 1:更地/2:古屋あり/10:古屋あり更地引渡可、建物: 1:居住中/2:空家/3:賃貸中/4:未完成）

### 今日完了した作業（2026-02-24 Seg 26）

- **Seg 26: スペーシング圧縮**（コミット: faafb11）
  - PropertyEditDynamicPage.tsx: py-8→py-4、mb-8→mb-4、p-6→p-3
  - DynamicForm.tsx: タブコンテンツ padding16→12/marginTop16→8/borderRadius12→8、グループ間gap16→8、タブタイトルアイコン32→20/見出し20→16、ナビゲーション marginTop12→8/padding16→10/borderRadius12→8
  - FormHeader.tsx: paddingTop16→8、タブボタンpadding10x16→6x12/borderRadius8→6
  - FieldGroup.tsx: marginBottom20→10/padding16→10/borderRadius12→8、ヘッダーmarginBottom16→8、アイコン24→18/見出し18→15、グリッドgap16→10、チェックボックスgap16→8
  - FieldFactory.tsx: フィールドmarginBottom16→8、チェックボックスpadding8→4
  - fieldUtils.ts: 入力欄padding12→6/fontSize15→14
  - 本番: デプロイ済み（GitHub Actions run #22336176468）
...
- **Seg 24a: zoho_id制約即時修正**（DB操作のみ、コミットなし）
  - UNIQUE制約DROP、NOT NULL DROP、DEFAULT ''→NULL、既存''→NULL変換
  - 新規物件作成のUNIQUE VIOLATION解消

- **Seg 24b: ZOHO CRM連携完全削除**（コミット: 35d8edb）
  - 7ファイル削除: zohoサービス4ファイル(903行) + zoho.py(614行) + zohoService.ts(107行) + ZohoImportPage.tsx(360行)
  - 9ファイル編集: api.py, config.py, exceptions.py, integrations.py, App.tsx, PropertyEditDynamicPage.tsx, constants.ts, apiPaths.ts, check_protocol.py
  - DB: zoho_id/zoho_synced_at/zoho_sync_status カラムDROP、zoho_import_staging等3テーブルDROP、column_labels 3行DELETE
  - 合計: -2,114行
  - 本番: デプロイ済み（GitHub Actions run #22272310833）
  - 残: 本番.envからZOHO_*5行を手動削除（動作には影響なし）
...
- **Seg 22: RegulationPanel表示/反映値一致修正**（コミット: 0298b3f）
  - 問題: パネル●項目がAPI生テキスト表示（例:「都市計画区域」）だが、フォーム反映値は別のラベル（例:「非線引区域」）→ユーザーに「嘘をつかれた」印象
  - reinfolib.py: _get_code_mappingでoption_valueも取得、labelsフィールドをレスポンスに追加
  - reinfolib.py: _convert_to_codesが(codes, labels)タプルを返却
  - RegulationPanel.tsx: RegulationLabels型追加、●項目はlabels優先表示（labels[field] || rawApiValue）
  - 本番: デプロイ済み（GitHub Actions run #22271284831）

### 今日完了した作業（2026-02-22 デプロイ: Seg 21）

- **Seg 21: 新規物件作成tax_type INTEGERバグ修正**（コミット: 00c4860）
  - 原因: PropertyEditDynamicPage.tsxのデフォルト値 `tax_type: '税込'`（ラベル文字列）→ INTEGER列にINSERTできずDBエラー
  - constants.ts: TAX_TYPE定数追加（TAX_INCLUDED=1, TAX_EXCLUDED=2, NO_DISPLAY=3）
  - constants.ts: PRICE_STATUS定数追加（NEGOTIABLE=1, FIXED=2, BIDDING=3）
  - PropertyEditDynamicPage.tsx: `tax_type: '税込'`→`TAX_TYPE.TAX_INCLUDED`、`price_status: '1'`→`PRICE_STATUS.FIXED`
  - fill_dummy_property.py: テストデータのラベル→数値修正
  - 本番: デプロイ済み（GitHub Actions run #22268547081）

### 今日完了した作業（2026-02-22 デプロイ: Seg 20）

- **Seg 20: 不要フィールド物理削除**（コミット: 3c4c165）
  - column_labels DELETE 14件: properties(geom, zoho_sync_status), land_info(land_law_permission), property_images(11件、imagesは残す)
  - DBカラムDROP 3件: properties.geom, land_info.land_law_permission, property_images.uploaded_at
  - images.py: uploaded_at参照7箇所削除（SELECT/INSERT/RETURNING/レスポンス）
  - generic.py: uploaded_at参照1箇所削除
  - propertyService.ts: uploaded_at型定義削除
  - ディスク容量不足対応: 古いバックアップ29件削除（約26GB解放）
  - 本番: デプロイ済み（GitHub Actions run #22268064756）

### 今日完了した作業（2026-02-22 デプロイ: Seg 19b）

- **Seg 19b: GeoPanel UIラベル「地図確定＋周辺取得」統一**（コミット: 6b5029f）
  - GeoPanel.tsx: ヘッダー「周辺情報を自動取得」→「地図確定＋周辺取得」
  - GeoPanel.tsx: 一括取得ボタン「この位置で周辺情報を一括取得」→「この位置で一括取得」
  - LocationField.tsx: オープンボタン「周辺情報を自動取得（学区・駅・バス・施設）」→「地図確定＋周辺取得（学区・駅・バス・施設）」
  - LocationField.tsx: 座標未設定メッセージ「周辺情報の自動取得が使えます」→「地図確定＋周辺取得が使えます」
  - 本番: デプロイ済み（GitHub Actions run #22267592514）
